...
//                .forEach(System.out::println);
        ...
        var maleStudents = Arrays.stream(students).filter(s -> s.getGender().equals("M"));

        System.out.println("Number of male students: " + maleStudents.count());

        for (String gender : List.of("M", "F")) {
            var myStudents = Arrays.stream(students).filter(s -> s.getGender().equals(gender));
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            total += cnt;
            System.out.printf("Number of students (%s) = %d%n", i == 0 ? " < 30" : ">= 30 & < 60", cnt);
        }

        System.out.println("Number of students >= 60 = " + (students.length - total));
